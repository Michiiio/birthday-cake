...
# Set up the screen and pen for fastest drawing
screen = trtl.Screen()
screen.bgcolor("#d3dae8")
screen.setup(900, 800)
# screen.title("Happy Birthday To MEEE")  # Trinket does not support this
...
def draw_ellipse(t, width, height, color, fill_color, y_offset=0):
    ...
    t.pencolor(color)
    t.begin_fill()
    for deg in range(0, 360, 16):  # Increase step for fewer points (was 8)
...
def draw_candle(t, x_pos, y_base, height):
    ...
    t.begin_fill()
    for deg in range(0, 360, 24):  # Increase step for fewer points (was 12)
        x = ellipse_x(4, deg) + x_pos
        y = ellipse_y(1, deg) + y_base
        t.goto(x, y)
    t.goto(x_pos + 4, y_base + height)
    for deg in range(0, 540, 24):  # Increase step for fewer points (was 12)
        x = ellipse_x(4, deg) + x_pos
...
pen.pendown()
pen.begin_fill()
for deg in range(0, 180, 2):  # Increase step for fewer points
    x = ellipse_x(150, -deg)
    y = ellipse_y(70, -deg)
    pen.goto(x, y)
for deg in range(180, 360, 2):  # Increase step for fewer points
    x = ellipse_x(150, deg)
    y = ellipse_y(60, deg)
    pen.goto(x, y)
...
for deg in range(0, 540, 4):  # Increase step for fewer points
    x = ellipse_x(120, deg)
...
pen.pensize(4)
pen.pencolor(color_palette["light_steel"])
for i in range(0, 1800, 4):  # Increase step for fewer points
...
pen.penup()
pen.goto(0, -120)  # Centered horizontally, just below the cake for all screens
pen.pencolor("black")
pen.pendown()
pen.write("HAPPY BIRTHDAY TO YOU", align="center", font=("Arial", 20, "bold"))
